import { mergeOverrideIgnores } from './file-filters.js';
...
function summarize(results, rules, written) {
    const selectedMessages = results.flatMap(result =>
        result.messages.filter(message => rules.has(message.ruleId)),
    );

    return {
        written,
        changedFileCount: results.filter(result => typeof result.output === 'string').length,
        remainingSelectedCount: selectedMessages.length,
        fatalCount: results.reduce(
            (count, result) => count + result.messages.filter(message => message.fatal).length,
            0,
        ),
    };
}
...
export async function fixRules(files, options = {}) {
    ...
    let eslintOptions = { ...options.eslintOptions };

    if (options.cwd) {
        eslintOptions.cwd = options.cwd;
    }

    eslintOptions = mergeOverrideIgnores(eslintOptions, options.ignorePatterns ?? []);
    ...
    return {
        ...
        rules: [...rules],
        files: fileList,
        summary: summarize(results, rules, written),
    };
}
