...
export function isExpectedCompatibilityError(error) {
    return error instanceof Error && (
        error.message.includes('ESLint is not installed next to eslint-rule-specific-fix') ||
        /ESLint .+ requires Node\.js/.test(error.message)
    );
}
...
export function normalizeRuntimeError(error) {
    ...
    if (isExpectedCompatibilityError(error)) {
